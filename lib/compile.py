...
gflags.DEFINE_integer(
    "recursion_limit",
    1000000,
    "The recursion limit to set " "for the Python interpreter.",
)
gflags.DEFINE_boolean(
    "gc", True, "If False (or --nogc), disables usage of " "Boehm GC"
)
...
def write(thing, name, mutable_vars):
    ...
    if type(thing) == Variable:
        ...
        if thing in mutable_vars:
            return "(*(((struct env_%s*)env)->%s).cell.addr)" % (
                name,
                thing.name,
            )
    ...
    if type(thing) == Begin:
        ...
        return "((%s = %s), %s)" % (
            helper(thing.instructions[0].var),
    ...
    if type(thing) == Lambda:
        return "MakeClosure(&&%s, alloc_env_%s(%s))" % (
            thing.name,
            thing.name,
            ", ".join(
                write(arg, name, set()) for arg in thing.freeVariables()
            ),
        )
    raise LogicError("shouldn't have gotten here")
...
def compile(input):
    ...
    for name in lambdas:
        ...
        for arg in list(lambdas[name].args) + list(
            lambdas[name].freeVariables()
        ):
        ...
        out.write(
            "\tstruct env_%s* t = GC_MALLOC(sizeof(struct env_%s));\n"
            % (name, name)
        )
        for arg in lambdas[name].freeVariables():
            out.write("\tt->%s = %s;\n" % (arg.name, arg.name))
    ...
    for name in lambdas:
        ...
        for i, arg in enumerate(lambdas[name].args):
            if arg in mutable_vars:
                out.write(
                    "\t((struct env_%s*)env)->%s = MakeCell(args[%d]);\n"
                    % (name, arg.name, i)
                )
            else:
                out.write(
                    "\t((struct env_%s*)env)->%s = args[%d];\n"
                    % (name, arg.name, i)
                )
        out.write(
            "\t%s;\n"
            % write(lambdas[name].instruction, name, mutable_vars)
        )
        out.write(CALLFUNC)
