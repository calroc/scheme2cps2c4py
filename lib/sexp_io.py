...
def read(lst):
    ...
    while True:
        if lst.eof():
            break
        if not WHITESPACE.match(lst.head()):
            break
        lst = lst.tail()

    # anything to read?
    if lst.eof() or lst.head() in (")", "]"):
        return None, lst
    ...
    for start_char, end_char in (("(", ")"), ("[", "]")):
        if lst.head() == start_char:
            ...
            if lst.head() != end_char:
                raise ParserError(
                    "Line %d: Missing %s" % (lst.lineno(), end_char)
                )
    ...
    # integers
    if (
        lst.head() == "-" and lst.tail().head().isdigit()
    ) or lst.head().isdigit():
        val = lst.head()
        while True:
            lst = lst.tail()
            if not lst.head().isdigit():
                break
            val += lst.head()
        ...
        while True:
            ...
            if lst.head() == "\\" and not lst.tail().eof():
                ...
                if lst.head() == "n":
                    ...
                    raise ParserError(
                        "Line %d: Invalid escape sequence: "
    ...
    if lst.head() == "#" and lst.tail().head().lower() in ("f", "t"):
        return (
            Token("boolean", lst.tail().head().lower() != "f"),
            lst.tail().tail(),
        )
    ...
    while True:
        ...
        if lst.head() == "#" and lst.tail().head() == "!":
            break
        if WHITESPACE.match(lst.head()):
            break
        if lst.head() == "#" and lst.tail().head() == "\\":
            lst = lst.tail().tail()
            if lst.eof():
                raise ParserError(
                    "Line %d: Missing character" % lst.lineno()
                )
        val += lst.head()
        lst = lst.tail()
    if not val:
        raise LogicError("no symbol?")
...
def parse(input):
    ...
    if not lst.eof():
        raise LogicError("didn't parse whole file?")
    return sexps
