...
class Variable(Exp):
    ...
    def __hash__(self):
        return hash((self.name, self.three_d))

    def __cmp__(self, other):
        if type(self) != type(other):
            return cmp(type(self), type(other))
        return cmp(
            (self.name, self.three_d), (other.name, other.three_d)
        )

    def freeVariables(self):
        if (
            self.name in PRIMITIVE_NAMES or self.name == "halt"
        ) and self.three_d:
            return set()
        return set([self])
...
class Application(Exp):
    ...
    def __str__(self):
        return "(%s)" % " ".join(
            [str(self.function)] + [str(x) for x in self.args]
        )

    def special(self):
        return (
            type(self.function) == Variable
            and self.function.three_d
            and self.function.name in PRIMITIVE_NAMES
        )

    def isTrivial(self):
        if self.special():
            return (
                not self.function.three_d
                or self.function.name != "call/cc"
            )
        return False
...
class Begin(Exp):
    ...
    def __str__(self):
        return "(%s)" % " ".join(
            ["begin"] + [str(x) for x in self.instructions]
        )
...
class If(Exp):
    ...
    def freeVariables(self):
        return (
            self.test.freeVariables()
            .union(self.true.freeVariables())
            .union(self.false.freeVariables())
        )

    def allVariables(self):
        return (
            self.test.allVariables()
            .union(self.true.allVariables())
            .union(self.false.allVariables())
        )

    def mutableVariables(self):
        return (
            self.test.mutableVariables()
            .union(self.true.mutableVariables())
...
class Lambda(Exp):
    ...
    def freeVariables(self):
        if not self._free_cache[1]:
            ...
            self._free_cache = [
                self.instruction.freeVariables() - bound,
                True,
            ]
...
class SetBang(Exp):
    ...
    def mutableVariables(self):
        return (
            self.var.mutableVariables()
            .union(self.val.mutableVariables())
            .union(set([self.var]))
        )
...
def quote(node):
    if type(node) == Token:
        if node.type == "symbol":
            return Variable(node.value, False)
...
def structure(node):
    ...
    if type(node[0]) == list:
        return Application(
            structure(node[0]), *list(map(structure, node[1:]))
        )
    ...
    if node[0].isSymbol("if"):
        assert len(node) == 4
        return If(
            structure(node[1]), structure(node[2]), structure(node[3])
        )
